...
def covariance_wind(f, c0, rho, distance, L, Cv, steps=10, initial=0.001):
    ...
    A = 5.0 / (
        18.0 * np.pi * gamma(1.0 / 3.0)
    )  # Equation 11, see text below. Approximate result is 0.033
    gamma_v = (
        3.0
        / 10.0
        * np.pi ** 2.0
        ...
        * 4.0
        * (Cv / c0) ** 2.0
    )  # Equation 28, only wind fluctuations

    krho = k * rho

    t = krho[:, None] * np.linspace(
        0.00000000001, 1.0, steps
    )  # Fine discretization for integration

    # t[t==0.0] = 1.e-20

    gamma56 = gamma(5.0 / 6.0)
    bessel56 = besselk(5.0 / 6.0, t)
    bessel16 = besselk(1.0 / 6.0, t)

    integration = cumtrapz(
        ne.evaluate("2.0**(1./6.)*t**(5./6.)/gamma56 * (bessel56 - t/2.0 * bessel16 )"),
        initial=initial,
    )[:, -1]
